...
defmodule Ex04 do
  ...
  def min(list), do: reduce(list,
    fn(val, curr_min) -> min(val, curr_min) 
  end)
  ...
  def even_odd(list), do: reduce(reverse(list), {[],[]},
    fn(val, new_tuple) -> split(Integer.is_even(val), val, new_tuple) 
  end)
  def split(true, val, {even, odd}), do: { [val|even], odd}
  def split(false, val, {even, odd}), do: { even, [val|odd]}
